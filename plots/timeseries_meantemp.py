...
for filename in os.listdir(directory):
    if filename.endswith('.csv'):
        ...
        year_counts = data.groupby(data['DATE'].dt.year)['MEAN TEMP'].count()
        ...
        for period in periods:
            ...
            if not period_data['DATE'].dt.year.empty and not period_data['MEAN TEMP'].empty:
                z = np.polyfit(period_data['DATE'].dt.year, period_data['MEAN TEMP'], deg=1)
        ...
        for station in weather_stations:
            if station['station_id'] == int(station_id):
                station['trend_lines'] = trend_lines
                break
